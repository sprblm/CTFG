<li class="nav-item has-submenu <?php if(@$activeCat == $sub_items->name ) { echo 'active'; } ?>">
    ...
    @if(count($sub_items->items) > 0)
        <a class="nav-link submenu-button <?php if(@$activeCat == $sub_items->name ) { echo 'menu-open'; } ?>">
    ...
    @endif
    @if ($sub_items->items)
        <ul class="submenu collapse <?php if((@$activeParent->name == $sub_items->name) || (@$activeCat == $sub_items->name) ) { echo 'show'; } ?>">
            @if(count($sub_items->items) > 0)
